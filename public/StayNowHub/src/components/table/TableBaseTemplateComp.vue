<template>
  <ion-card>
    <ion-card-content>
      <ion-grid>
        <ion-row>
          <ion-col  :class="['title-header', { 'last-header': index === headers.length - 1 }]" v-for="(header, index) in headers" :key="header">
            <strong>{{ header }}</strong>
          </ion-col>
        </ion-row>
        <ion-row v-for="(row, rowIndex) in data" :key="rowIndex">
          <ion-col :class="['item-text', { 'total-table': cellIndex ===  1 }]" v-for="(cell, cellIndex) in row" :key="cellIndex">
