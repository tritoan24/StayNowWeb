...
async function fetchAllPaymentHistoris() {
  ...
  try {
    ...
    renderRoomList(paymentHistoryData);
  } catch (e) {
    console.error("Lỗi khi lấy danh sách lich su:", e);
  }
}


function renderPaymentHistoryList(paymentHistoryData) {
  const paymentHistoryListContainer = document.getElementById("paymentHistoryList");
  paymentHistoryListContainer.innerHTML = ""; // Xóa nội dung cũ


  paymentHistoryData.forEach((paymentHistory) => {
    const paymentHistoryDiv = document.createElement("div");
    paymentHistoryDiv.className = "paymentHistory";
    paymentHistoryDiv.id = `paymentHistory${paymentHistory.id}`;

    paymentHistoryDiv.innerHTML = `
      <div class="payment-history-card">
        <div class="payment-history-image">
            <img src="${paymentHistory.Icon_dichvu}" alt="${paymentHistory.Ten_dichvu}" />
        </div>
        <div class="payment-history-info">
            <h3 class="payment-history-title">${paymentHistory.Ten_dichvu}</h3>
           
        </div>
      </div>
    `;
    paymentHistoryListContainer.appendChild(paymentHistoryDiv);
  });
